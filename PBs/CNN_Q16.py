...
def get_arguments():
    ...
    parser.add_argument('--loss', type=str, default='mse_loss', help="Функция потерь") # This arg is not used directly for CrossEntropyLoss
...
def extract_cnn_model_info(folder_path):
    ...
    for file_path in folder.glob("*"):
        ...
        if file_path.is_file() and "GNN" not in file_path.name and "_FOR_CPU" not in file_path.name:
            ...
            # This regex needs to match the new `current_index` format
...
def save_data_and_calculate_accuracy(
    ...
    with torch.no_grad():
        ...
        data = {
            ...
            "loss_function": "CrossEntropyLoss", # Hardcoded, assuming it's always CrossEntropyLoss
        ...
        trained_models_folder = Path("./trained_models")
        trained_models_folder.mkdir(parents=True, exist_ok=True) # Ensure folder exists

        trained_models = extract_model_info_func(trained_models_folder)
        ...
        for file_path, index, accuracy_from_filename in trained_models:
            ...
            if index == current_index and file_path.name.startswith(Path(LastModelName).name.split('/')[-1]):
        ...
        if val_accuracy * 100 > existing_last_model_accuracy:
            ...
            # Ensure the directory exists for LastModelName
            last_model_dir = Path(LastModelName).parent
            last_model_dir.mkdir(parents=True, exist_ok=True)

            torch.save(model, f'{LastModelName}_{current_index}_{val_accuracy * 100:.2f}.pth')
            torch.save(model.state_dict(), f'{LastModelName}_{current_index}_{val_accuracy * 100:.2f}_FOR_CPU.pth')
        ...
        if val_accuracy * 100 > overall_best_accuracy:
            ...
            torch.save(model, f'{model_prefix}_{current_index}_{val_accuracy * 100:.2f}.pth')
            torch.save(model.state_dict(), f'{model_prefix}_{current_index}_{val_accuracy * 100:.2f}_FOR_CPU.pth')
...
# Model Definition Class (remains outside __name__ block for importability)
class MultiOutputRegression(nn.Module):
    def __init__(self, freeze_conv=False, input_len=380, num_outputs=16, args=None):
        super().__init__()
        # Use args directly here, or pass specific required arguments for clarity
        if args is None:
            # Fallback for when called without args (e.g., for `temp_model` in pretrained loading)
            # This is a bit of a tricky dependency. Ideally, Model should not directly use global `args`.
            # For this context, it is needed.
            print("Warning: MultiOutputRegression initialized without 'args'. Using hardcoded defaults.")
            self.conv_layers = 2
            self.fc_layers = 1
            self.kernel_size = 10
            self.stride = 2
            self.padding = 1
            self.first_layer_channels = 32
            self.conv_activation = activations['tanh']
            self.fc_activation = activations['sigmoid']
            # Assume num_outputs and input_len are always fixed
            ...
            self.first_layer_channels = args.first_layer_channels
            self.conv_activation = activations[args.conv_activation]
            self.fc_activation = activations[args.fc_activation]
            self.num_outputs = num_outputs # Fixed at 16 for Q16 problem
            self.input_len = input_len # Fixed at 380
        ...
        # This will be passed to init or derived from data later
        num_classes = self.num_outputs # Should be 16 for Q16
        powers = np.linspace(int(np.floor(np.log2(l - 1))), int(np.ceil(np.log2(num_classes + 1))), self.fc_layers - 1)
...
if __name__ == "__main__":
    ...
    print(f"Current Model Index: {current_index}")
    ...
    # Load data
    try:
        X = np.load(f"./{db_folder_name}/train_Q16X.npy").astype('float32')
        y = np.load(f"./{db_folder_name}/train_Q16Y.npy").astype('float32')
        ...
        print(f"Error loading data file: {e}. Please ensure all .npy files are in the correct directories.")
        sys.exit(1)

    # Reshaping input data
    # Ensure all inputs are (samples, 1, features) for Conv1D
    ...
    if previous_model_yn.lower() == 'y': # Use .lower() for case-insensitive check
        print('Type in index of a model you want to take as a basis for training (e.g., 2_1_10_2_1_32_tanh_sigmoid)')
        model_index = input()

        state_dict_path = Path(f'{LastModelName}_{model_index}_FOR_CPU.pth') # Ensure .pth extension for consistency

        if state_dict_path.exists():
            old_state = torch.load(state_dict_path, map_location=device) # Load to current device
            # Instantiate temp_model with the *current* arguments for shape comparison
            temp_model = MultiOutputRegression(input_len=X.shape[-1], num_outputs=y.shape[-1], args=args).to(device)
            temp_state = temp_model.state_dict()
            hyperparameters_match = check_hyperparameter_match(old_state, temp_state)

            model = MultiOutputRegression(freeze_conv=not hyperparameters_match,
                                          input_len=X.shape[-1], num_outputs=y.shape[-1], args=args).to(device)
            ...
            try:
                ...
                print("Invalid input for skip epochs. Skipping 0 epochs.")
                skip_epochs = 0
        else:
            raise ValueError(f"No state_dict file found at: {state_dict_path}. Please check the path and filename.")
    elif previous_model_yn.lower() == 'n':
        model = MultiOutputRegression(input_len=X.shape[-1], num_outputs=y.shape[-1], args=args).to(device)
        skip_epochs = 0
    else:
        print("Invalid input for --pretrained. Use 'Y' or 'n'. Exiting.")
        sys.exit(1)
    ...
    epochs = args.epochs
    best_accuracy = 0 # This variable isn't used to track overall best, can be removed if not needed.
    kk = 0 # Iteration counter for progress updates
    ...
    for epoch in range(epochs):
        ...
        if epoch < skip_epochs:
            print(f"Skipping epoch {epoch + 1}/{epochs} as requested.")
            kk += len(train_loader) # Still increment kk for consistent logging if needed
        else:
