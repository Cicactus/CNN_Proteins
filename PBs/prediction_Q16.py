...
def main():
    ...
    # Assuming filename format like 'PREFIX_convlayers_fclayers_kernelsize_stride_padding_firstlayerchannels_accuracy_FOR_CPU'
    # Adjust indexing if your filename format differs.
    try:
        ...
        print(f"Error parsing model architecture from filename: {e}. Ensure filename matches expected pattern (e.g., Q16_BEST_MODEL_2_1_10_2_1_32_...). Using default parameters.")
    ...
    for data_path in args.dataset_paths:
        ...
        # Construct Y_path based on X_path (e.g., change '.npy' to 'Y.npy' and remove 'X' from name)
        Y_path_base = data_path.replace('X.npy', 'Y.npy')
        Y_path = Y_path_base
        
        # Handle cases where Y_path might be in 'alldb' or root, similar to how X is loaded
        # The training script loads train_Q8Y.npy from root, and others from alldb.
        # This inference script needs to handle both cases too.
        if "CB513X.npy" in data_path:
            Y_path = "./CB513Y.npy" # Special case for CB513, assumed to be in root
        elif "test_for_dsspX.npy" in data_path:
            Y_path = f"./{db_folder_name}/test_for_dsspY.npy"
        elif "validation_for_dsspX.npy" in data_path:
            Y_path = f"./{db_folder_name}/validation_for_dsspY.npy"
        elif "train_Q8X.npy" in data_path:
            Y_path = "./train_Q8Y.npy" # Assuming train_Q8Y.npy is in root
