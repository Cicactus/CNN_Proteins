...
def find_and_load_q8_index_params(folder_path=".", model_prefix="Q8"):
    """
    Finds the first model that starts with 'Q8' in the specified folder,
    extracts its index values from the filename, and returns them along with the full path to the model file.
    """
    folder = Path(folder_path)

    # Regex to capture all parts of the Q8 model filename
    ...
    for file_path in folder.iterdir():
        ...
        if file_path.is_file() and file_path.name.startswith(model_prefix):
            match = q8_model_regex.match(file_path.name)
            if match:
                # Extract values
...
class Model(nn.Module):
    def __init__(self, conv_layers, fc_layers, kernel_size, stride, padding,
                 ...
                 fc_activation_name='leakyrelu', n_outputs=8):
        super().__init__()
...
(conv_layers, fc_layers, kernel_size, stride, padding,
 first_layer_channels, conv_activation, fc_activation, accuracy, model_file_to_load) = \
    find_and_load_q8_index_params(folder_path=".") # Adjust folder_path as needed
...
model = Model(
    ...
    n_outputs=8 # Assuming n_outputs is always 16 for Q16 models
)
