...
from torch.utils.data import DataLoader, TensorDataset, Dataset # Keep Dataset and TensorDataset

#  Global Constants 
# These were likely defined or imported from general_functions.py
...
def get_arguments():
    ...
    parser.add_argument('--loss', type=str, default='mse_loss', help="Loss function")
...
# These were also likely defined or imported from general_functions.py
activations = {
...
def create_dataloader(X_data, y_data, batch_size):
    # X_data will be float32, y_data will be float32 (RMSD values)
...
def calculate_classification_accuracy(y_pred_logits, true_labels_class_indices):
    ...
    Args:
        y_pred_logits (torch.Tensor): Model outputs (raw logits) of shape (batch_size, num_classes).
        true_labels_class_indices (torch.Tensor): True labels (integer class indices) of shape (batch_size,).
...
def get_loader_accuracy_CNN(loader, model, device):
    ...
    with torch.no_grad():
        ...
        for Xt_batch, yt_batch_rmsd in loader: # Renamed for clarity, yt_batch_rmsd contains RMSD values
            ...
            # NOTE: Changed from argmin to argmax in this context based on the input code's handling of y_train_indices_for_counts.
            # If y_train_indices_for_counts is using argmax, then here too.
            true_labels_class_indices = torch.argmax(yt_batch_rmsd, dim=1).long().to(device)
...
def extract_cnn_model_info(folder_path):
    ...
    for file_path in folder.glob("*"):
        ...
        if file_path.is_file() and "GNN" not in file_path.name and "_FOR_CPU" not in file_path.name:
            # Regex to match index and accuracy at the end of the filename
            match = re.search(r"(\d+(?:_\d+)*)_(\d+\.\d+)$", file_path.name)
            if match:
...
current_index = f'{args.conv_layers}_{args.fc_layers}_{args.kernel_size}_{args.stride}_{args.padding}_{args.first_layer_channels}'
...
CB513X = np.load(f"./CB513X.npy").astype('float32')
CB513Y = np.load(f"./CB513Y.npy").astype('float32')
...
class MultiOutputRegression(nn.Module):
    def __init__(self, freeze_conv=False):
        ...
        num_classes = y.shape[-1] # This is correct, as y has a column for each "class"
    ...
    def forward(self, x):
        ...
        x = nn.Softmax(dim=1)(self.fcs[-1](x))
        return x
...
for epoch in range(epochs):
    ...
    if epoch < skip_epochs:
        ...
        for step, (Xt_batch, yt_batch_rmsd) in enumerate(train_loader):
            ...
            yt_batch_class_indices = torch.argmax(yt_batch_rmsd, dim=1).long().to(device)
